...
export default {
  ...
  methods: {
    async fetchCandidates(pageToFetch = this.currentPage) {
      ...
      let url = `${process.env.VUE_APP_BACKEND_URL}/candidates/search?page=${this.currentPage}&size=${this.pageSize}`;

      if (this.searchQuery.trim() !== "") {
        url += `&search=${encodeURIComponent(this.searchQuery.trim())}`;
      } else {
        url = `${process.env.VUE_APP_BACKEND_URL}/candidates?page=${this.currentPage}&size=${this.pageSize}`;
      }
